...
export function InnerStrip({
  ...
  innerStripWidth, // Ширина новой оранжевой полосы
  innerStripPadding, // Отступ оранжевой полосы от внутренней границы желтой
}: {
  ...
  const generateStripGeometry = useCallback(() => {
    ...
    // Правильные смещения для оранжевой полосы
    // Оранжевая полоса должна быть внутри желтой полосы
    // При innerStripPadding = 0 оранжевая полоса должна касаться внутренней границы желтой полосы
    const outerOffsetOrange = stripWidth + innerStripPadding - innerStripWidth // Внешний край оранжевой полосы
    const innerOffsetOrange = stripWidth + innerStripPadding // Внутренний край оранжевой полосы
    ...
    for (let k = 0; k < numPoints; k++) {
      ...
      // Calculate outer edge of orange strip
      ...
      // Calculate inner edge of orange strip
      if (isPrevSideActive && isCurrSideActive) {
        // Both sides active: calculate intersection
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      vertices.push(
        outerStart.x,
        outerStart.y,
        0.01, // Z slightly above yellow strip
        outerEnd.x,
        outerEnd.y,
        0.01,
        innerEnd.x,
